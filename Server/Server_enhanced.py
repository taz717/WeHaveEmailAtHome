    ...
    while 1:
        try:
            ...
            if pid == 0:
                ...
                for user in userdata:
                    if (user == username):
                        if (password == userdata[username]["password"]):
                            validUser = 1
                            if (userdata[username]["status"] == 0):
                                change_state(userdata, username, 1)
                            else:
                                validUser = 2
                ...
                if (validUser == 1):
                    ...
                    while (1):
                        ...
                        if (menuChoice == "1"):
                            ...
                            change_state(userdata, username, 0)
                            break
                ...
                if (validUser == 2):
                    # Invalid user, send termination notice
                    limitMsg = "Entered username is already logged in"
                    connectionSocket.send(limitMsg.encode('ascii'))
                    print(
                        f"The received client information: {username} is already logged in (Connection Terminated).")
